...
async def get_message(user: User = Depends(current_user), chat: ChatRead = Depends(get_chat_by_chat_id),
                      session: AsyncSession = Depends(get_session)):
...
async def send_message(message: MessageCreate, user: User = Depends(current_user),
                       chat: ChatRead = Depends(get_chat_by_chat_id),session: AsyncSession = Depends(get_session)):
